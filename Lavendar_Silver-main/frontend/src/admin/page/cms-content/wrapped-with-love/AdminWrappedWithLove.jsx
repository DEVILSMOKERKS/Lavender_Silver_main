...
const AdminWrappedWithLove = () => {
  ...
  const fetchBanners = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/home-banners/wrapped-love`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalType === 'add') {
        await axios.post(`${API_BASE_URL}/api/home-banners/wrapped-love`, formData, config);
      } else if (modalType === 'edit' && currentBanner) {
        await axios.put(`${API_BASE_URL}/api/home-banners/wrapped-love/${currentBanner.id}`, formData, config);
      }
      fetchBanners();
      closeModal();
    } catch (err) {
      // Log the full error for debugging
      console.error('Wrapped with love banner API error:', err, err.response);
  ...
  const confirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${API_BASE_URL}/api/home-banners/wrapped-love/${bannerToDelete.id}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  const handleDrop = async (e, dropIndex) => {
    ...
    try {
      await axios.put(
        `${API_BASE_URL}/api/home-banners/wrapped-love/positions/update`,
  ...
  return (
    <div className="admin-wrap-love-root">
      <div className="admin-wrap-love-header">
        <h1 className="admin-wrap-love-title">Wrapped With Love Banners</h1>
        <div className="admin-wrap-love-controls">
          <div className="admin-wrap-love-search-container">
            <Search className="admin-wrap-love-search-icon" />
            <input
              type="text"
              placeholder="Search Images..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="admin-wrap-love-search-input"
            />
          </div>
          <button className="admin-wrap-love-add-btn" onClick={() => openModal('add')}>
            <Plus className="w-4 h-4" /> Add Images
          </button>
        </div>
      </div>

      {error && <div className="admin-wrap-love-error">{error}</div>}

      <div className="admin-wrap-love-table-container">
        {updatingPositions && (
        ...
        <table className="admin-wrap-love-table">
          <thead>
            <tr>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="10" className="admin-wrap-love-center"><Loader2 className="spin" /> Loading...</td></tr>
            ) : !Array.isArray(banners) || banners.length === 0 ? (
              <tr><td colSpan="10" className="admin-wrap-love-center">No banners found</td></tr>
            ) : (
              (banners || []).map((banner, index) => (
                ...
                >
                  ...
                  <td>{banner.category_name || 'N/A'}</td>
                  <td>{banner.position}</td>
                  <td>{banner.is_active ? 'Active' : 'Inactive'}</td>
                  <td>
                    <div className="admin-wrap-love-images">
                      {banner.image_url ? (
                        <img
                          src={
                            banner.image_url
                          ...
                          className="admin-wrap-love-thumb"
                        />
                      ) : (
                        <ImageIcon className="admin-wrap-love-noimg" />
                      )}
                    </div>
                  </td>
                  <td>
                    <button className="admin-wrap-love-action-btn" onClick={() => openModal('edit', banner)}><Edit className="w-4 h-4" /></button>
                    <button className="admin-wrap-love-action-btn" onClick={() => handleDeleteClick(banner)}><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))
            )}
      ...
      {/* Modal for Add/Edit */}
      {showModal && (
        <div className="admin-wrap-love-modal-overlay">
          <div className="admin-wrap-love-modal">
            <button className="admin-wrap-love-modal-close" onClick={closeModal}><X /></button>
            <h2>{modalType === 'add' ? 'Add Banner' : 'Edit Banner'}</h2>
            <form onSubmit={handleSubmit} className="admin-wrap-love-form">
              <label>Title
              ...
              <div className="admin-wrap-love-modal-actions">
                <button type="submit" className="admin-wrap-love-save-btn" disabled={loading || uploading}>
                  {loading ? <Loader2 className="spin" /> : (modalType === 'add' ? 'Add Banner' : 'Save Changes')}
                </button>
                <button type="button" className="admin-wrap-love-cancel-btn" onClick={closeModal}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showDeleteConfirm && bannerToDelete && (
        <div className="admin-wrap-love-delete-modal-overlay">
          <div className="admin-wrap-love-delete-modal">
            ...
            <div className="admin-wrap-love-delete-modal-actions">
...
export default AdminWrappedWithLove;
