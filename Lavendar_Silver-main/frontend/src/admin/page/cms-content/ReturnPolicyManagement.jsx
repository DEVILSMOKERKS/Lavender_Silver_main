...
import './ReturnPolicyManagement.css';
...
const ReturnPolicyManagement = () => {
  ...
  const fetchPolicyData = async () => {
    try {
      ...
      showNotification('Failed to load Return Policy data. Please try again.', 'error');
    }
  };
  ...
  const handleHeaderSubmit = async (e) => {
    ...
    try {
      if (hasHeaderData) {
        ...
        const response = await axios.put(`${API_BASE_URL}/api/return-policy/header/${policyData.header.id}`, headerForm, {
          headers: {
        ...
        // Create new header
        const response = await axios.post(`${API_BASE_URL}/api/return-policy/header`, headerForm, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
    ...
    try {
      if (hasSectionsData) {
        ...
        const promises = allSections.map(section =>
          axios.put(`${API_BASE_URL}/api/return-policy/sections/${section.id}`, section, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
        ...
        const promises = allSections.map(section =>
          axios.post(`${API_BASE_URL}/api/return-policy/sections`, section, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
  ...
  return (
    <div className="return-policy-management">
      <div className="return-policy-management-page-header">
        <h1>Return & Cancellation Policy Management</h1>
        <p>Manage Return & Cancellation Policy content for the website</p>
      </div>

      {/* Loading State */}
      {loading && <div className="return-policy-management-loading">Loading...</div>}

      {/* Tabs */}
      <div className="return-policy-management-policy-tabs">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            className={`return-policy-management-tab-button ${activeTab === tab.id ? 'active' : ''}`}
            onClick={() => setActiveTab(tab.id)}
          >
            <span className="return-policy-management-tab-icon">{tab.icon}</span>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {/* Tab Content */}
      {activeTab === 'header' && (
        <div className="return-policy-management-tab-content">
          <h2>Policy Header</h2>

          {/* Header Form */}
          <div className="return-policy-management-add-policy-section">
            <h3>{hasHeaderData ? 'Update Header' : 'Add Header'}</h3>
            {hasHeaderData && (
              <div className="return-policy-management-update-note">
                <p>📝 Header data exists. You can update the existing header below.</p>
              </div>
            )}
            <form onSubmit={handleHeaderSubmit}>
              <div className="return-policy-management-form-group">
                <label>Page Title</label>
                <input
                  type="text"
                  value={headerForm.page_title}
                  onChange={(e) => setHeaderForm({ ...headerForm, page_title: e.target.value })}
                  placeholder="Enter page title (e.g., RETURN & CANCELLATION POLICY)"
                  required
                />
              </div>
              <div className="return-policy-management-form-group">
              ...
              <div className="return-policy-management-form-actions">
                <button type="submit" disabled={loading} className="return-policy-management-submit-btn">
          ...
          {!hasHeaderData && policyData.header && (
            <div className="return-policy-management-existing-policy">
              <h3>Existing Header</h3>
              <div className="return-policy-management-policy-item">
          ...
          <div className="return-policy-management-add-policy-section">
            ...
            <form onSubmit={handleSectionsSubmit}>
              {sectionEntries.map((entry, index) => (
                <div key={index} className="return-policy-management-policy-entry-card">
                  <div className="return-policy-management-entry-header">
                    <h4>Main Section {index + 1}</h4>
                    {sectionEntries.length > 1 && (
                      <button
                        type="button"
                        onClick={() => removeSectionEntry(index)}
                        className="return-policy-management-remove-entry-btn"
                      >
                        ✕ Remove
                      </button>
                    )}
                  ...
                  <div className="return-policy-management-form-group">
                    ...
                    <input
                      ...
                      placeholder="Enter section title (e.g., RETURN POLICY, CANCELLATION POLICY)"
                      required
                    />
                  </div>
                  <div className="return-policy-management-form-group">
                  ...
                  {/* Subsections */}
                  <div className="return-policy-management-subsections-container">
                    <div className="return-policy-management-subsections-header">
                      <h5>Subsections</h5>
                      <button
                        type="button"
                        onClick={() => addSubsection(index)}
                        className="return-policy-management-add-subsection-btn"
                      >
                        ➕ Add Subsection
                      </button>
                    </div>
                    ...
                    {entry.subsections.map((subsection, subIndex) => (
                      <div key={subIndex} className="return-policy-management-subsection-card">
                        <div className="return-policy-management-subsection-header">
                          <h6>Subsection {subIndex + 1}</h6>
                          <button
                            type="button"
                            onClick={() => removeSubsection(index, subIndex)}
                            className="return-policy-management-remove-subsection-btn"
                          >
                            ✕ Remove
                          </button>
                        </div>
                        ...
                        <div className="return-policy-management-form-group">
                          ...
                          <input
                            ...
                            placeholder="Enter subsection title (e.g., Eligible Items for Returns)"
                            required
                          />
                        </div>
                        <div className="return-policy-management-form-group">
              ...
              <div className="return-policy-management-form-actions">
                {!hasSectionsData && (
                  <button type="button" onClick={addSectionEntry} className="return-policy-management-add-more-btn">
                    ➕ Add More Section
                  </button>
                )}
                <button type="submit" disabled={loading} className="return-policy-management-submit-btn">
            ...
            <div className="return-policy-management-existing-policy">
              ...
              {(() => {
                ...
                return mainSections.map((section, index) => {
                  const sectionSubsections = subsections.filter(sub => sub.parent_section_number === section.section_number);
                  return (
                    <div key={section.id} className="return-policy-management-policy-item">
                      <h4>{section.section_number} {section.section_title}</h4>
                      <p>{section.content}</p>

                      {/* Show subsections */}
                      {sectionSubsections.length > 0 && (
                        <div className="return-policy-management-subsections-display">
                          <h5>Subsections:</h5>
                          {sectionSubsections.map((subsection, subIndex) => (
                            <div key={subsection.id} className="return-policy-management-subsection-display">
...
export default ReturnPolicyManagement; 